    ...
    try {
      ...
      const data = await callOpenAI(
        [
          {
            role: 'system',
            content: `You are a pediatric dermatology expert and skincare educator specializing in creating age-appropriate skincare routines for children and teens. 
...
Always return valid JSON in the exact format requested.`
  ...
  async generateAndSaveTemplate(
    ...
    try {
      ...
      // Save to database
      const template = await routineTemplateService.createTemplate({
        type: request.type,
  ...
  async regenerateTemplate(
    ...
    try {
      const existing = await routineTemplateService.getTemplateById(templateId);
      ...
      const updated = await routineTemplateService.updateTemplate(templateId, {
        title: generated.title,
        description: generated.description,
        default_duration: generated.default_duration,
        default_instructions: generated.default_instructions,
