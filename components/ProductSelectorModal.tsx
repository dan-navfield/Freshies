...
export default function ProductSelectorModal({
  ...
  const loadProducts = async () => {
    setLoading(true);
    try {
      // Load popular/default products or recent scans
      // First try to get child's scanned products for recommendations
      ...
      console.log('Child scanned products:', scannedProducts?.length || 0);

      // Convert to our Product format
      const childProducts: Product[] = (scannedProducts || []).map(p => ({
        id: p.id,
